<template>
  <div class="flex flex-col justify-center items-center h-full overflow-hidden relative">
    ...
    <form class="flex flex-col items-center w-1/4 z-0" @submit.prevent="register()">
      ...
      >{{ emailError }}</span>
      <input type="password"
             placeholder="Ton super mot de passe (indice: utilise 12345)"
             class="w-full h-12 mb-8 pl-5 rounded-full"
             v-model="password"
             @input="checkPassword()"
      >
      <input type="password"
             placeholder="Confirme ton super mot de passe"
             class="w-full h-12 mb-1 pl-5 rounded-full"
      ...
      <router-link to="/login"><a class="text-blue">Déjà chez nous ?</a></router-link>
    </form>
  </div>
    ...
    checkFullName() {
      ...
      if (this.fullName) {
        if (this.fullName.length < 4) {
          this.fullNameError = "Faignasse ! Nom trop court !";
        } else if (this.fullName.length > 30) {
          this.fullNameError = "Oh ! Nom trop long ! "
        } else if (!/^[a-zA-Z\s]*$/.test(this.fullName)) {
          this.fullNameError = "Pas assez intelligent pour utiliser des caractères complexes ?";
        } else {
          this.fullNameError = "";
    ...
    checkEmail() {
      ...
      if (this.email) {
        ...
        if (!emailRegex.test(this.email)) {
          this.emailError = "Email incorrect";
        } else {
          this.emailError = "";
        ...
        if (this.password.length >= 8) {
          ...
          if (!hasNumber.test(this.password) || !regex.test(this.password)) {
            ...
            if (this.confirmPassword && this.password !== this.confirmPassword) {
              this.passwordError = "Regarde la définition de 'CONFIRMER' "
            } else {
              this.passwordError = "";
          ...
          this.passwordError = "Trop court ! Pourtant ... ";
        }
      } else {
        this.passwordError = "";
      }
